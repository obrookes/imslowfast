...
class Tap(torch.utils.data.Dataset):
    """
        ...
        with pathmgr.open(path_to_file, "r") as f:
            ...
            for clip_idx, path_label in enumerate(rows):
                ...
                if len(fetch_info) == 2:
                    ...
                    path, label = path_label.split(',"')[0], ast.literal_eval(
                        path_label.split(',"')[-1].replace('"', "")
                    )
                else:
                    raise RuntimeError(
        ...
        for i_try in range(self._num_retries):
            ...
            for i in range(num_decode):
                for _ in range(num_aug):
                    # TODO: reshape tensor to apply per clip augmentation
                    idx += 1

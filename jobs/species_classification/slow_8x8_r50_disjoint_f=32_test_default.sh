#!/bin/bash
#SBATCH --nodes=1
#SBATCH --gres=gpu:4
#SBATCH --ntasks-per-gpu=1
#SBATCH --cpus-per-task=72
#SBATCH --job-name=slow_8x8_r50_disjoint_f=32_test_default
#SBATCH --time=12:00:00
#SBATCH --output=%x.out

echo "Running on host $(hostname)"
echo "Time is $(date)"
echo "Slurm job ID is $SLURM_JOB_ID"

cd ~/imslowfast

# Purge
module purge

# Load CUDA
module load cudatoolkit/23.9_12.2

singularity exec --nv --bind /lus/lfs1aip1/home/aiape/obrookes.aiape:/mnt ./singularity/slowfast.sif python -W ignore ./tools/run_net.py \
    --cfg '/mnt/imslowfast/configs/species_classification/test/r50/SLOW_8x8_R50_DISJOINT_F=32_TEST_DEFAULT.yaml'
